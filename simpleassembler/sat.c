...
int encode_command(const char* mnemonic, int operand)
{
    ...
    if (strcmp(mnemonic, "NOP") == 0)
        opcode = CMD_NOP;
    else if (strcmp(mnemonic, "CPUINFO") == 0)
        opcode = CMD_CPUINFO;
    else if (strcmp(mnemonic, "READ") == 0)
        opcode = CMD_READ;
    else if (strcmp(mnemonic, "WRITE") == 0)
        opcode = CMD_WRITE;
        ...
        if (sscanf(line, "%s %s %s", addr_str, token, operand_str) == 3) {
            ...
            if (strcmp(token, "=") == 0) {
                ...
                if (sscanf(operand_str, "%d", &value) == 1) {
                    ...
                    fprintf(stderr,
                            "Invalid value for address %d: %s\n",
                            addr,
                            operand_str);
                }
            } else {
                int operand;
                if (sscanf(operand_str, "%d", &operand) == 1 && operand >= 0
                    && operand <= 127) {
